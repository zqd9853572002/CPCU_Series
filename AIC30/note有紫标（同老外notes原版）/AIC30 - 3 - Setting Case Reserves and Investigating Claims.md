3 – Setting Case Reserves and Investigating Claims
...
- individual case method中，claim rep会为each claim/ each cause of loss设定individual/ case reserve（基于claim rep认为保险公司会赔多少钱）
  
  - `对each claim使用individual case method时，claim rep会根据claim circumstance和similar claim experience来估算case reserve`
- 由于evaluation的subjective nature和claim rep要考虑的各种factor，使用individual case method为bodily injury claim建立reserve时，每个claim rep的reserve会有巨大差异

### （1）Individual Case Method中的Consideration

- 用individual case method设定bodily injury claim 的reserve时，claim rep会考虑如下因素

  |                                        | 用途                                             | 具体的factor                                                 | 案例                                                         |
  | -------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | Claimant Profile                       | `用于计算economic loss`                          | `Age<br/><br />Gender<br/><br />Occupation<br/><br />Level of education<br/><br />Dependent（age, 依赖claimant的financially/ companionship的extent）` | `disability不会影响earn living的ability，但其会影响其参加sport/ physical/ family outing（injury需考虑这些factor）` |
  | injury nature<br /><br />injury extent | 用于计算general damage                           | `injury是否是permanent/ disfiguring<br /><br />pain and suffering extent<br /><br />injury对individual lifestyle的disruption extent` | 和injury nature/ extent相关的包括permanent nerve damage<br /><br />infection会加剧pain and suffering extent<br /><br />要评估injury会在多大程度上影响其lifestyle（如影响和family的physical activity） |
  | special damage                         | 用于计算special damage                           | `anticipated medical bill（包括incurred to date和future care）<br /><br />已经提供或正在提供的medical care type（是否包括diagnostic care或treatment）<br /><br />claimant是否lose wage` | -                                                            |
  | Claimant representation                | 用于确定lawsuit likelihood，并预测general damage | `claimant是否有lawyer（如果有，lawyer的reputation）<br /><br />local court verdict的typical value` | -                                                            |
  | Liability factor                       | 用于计算compensatory damage和或punitive damage   | `case是否涉及ordinary negligence/ gross negligence<br /><br />是否涉及comparative negligence/ contributory negligence<br /><br />recovery是否有legal limit（如对特定的damage type有cap）<br /><br />是否有其他当事方要对loss负责，或contribute to settlement` | -                                                            |
  | Miscellaneous factor                   | -                                                | `geographic area的economic condition（用于计算economic loss）<br /><br />insured造成loss的行为是否属于outrageous（用于计算compensatory damage）<br /><br />drinking/ drug是否对loss有影响（用于计算liability）<br /><br />insured作为witness的credibility（用于确定successful lawsuit的likelihood）<br /><br />claimant作为witness的credibility（用于确定successful lawsuit的likelihood）` | -                                                            |

### （2）roundtable method

- `理想情况下，claim人员不会知道其他人设定的reserve`
  - `经过evaluation和discussion后，会对reserve figure达成consensus（或计算出all figure的average）`
- `roundtable method非常费时间（time consuming），所以其不适用于设定initial reserve，但对于serious/ prolonged claim，roundtable method是review initial reserve的suitable method`

### （3）average value method

- claim rep/ claim-processing computer system可使用average value method来设定claim reserve
- `average value method适用于loss size variation较小，且能快速conclude claim的case，如auto windshield claim`
- `average value基于past claim data，并进行相应adjustment以反映current condition`

- 有些claim的initial reserve基于average value method，但claim rep要在几天之内修改initial reserve，以反映each claim circumstance

### （4）formula method

- `formula method中，设定reserve的formula有保险公司确定，并会基于claim fact自动创建`

  > `formula可基于WC claim中medical cost和indemnity（wage loss）之间的ratio`

- `根据保险公司similar claim 的loss history，indemnity reserve会设定为medical reserve的certain percentage`

- formula method可用于设定HO policy中的additional living expense reserve（此时reserve通常是coverage limit的certain percentage）
...
- `和formula method类似，expert system包括subjective information（如loss location和treating physician name等）`
...
- `claim rep可采取的action包括`
  ...
  - `reject position前，通过independent source来检查insured提供的fact`
    ...
    - `如果两个account之间有conflict，claim rep需要联系其他witness（作为independent source），以confirm/ deny insured version of the fact`
      ...
      > A说，E（insured）故意伤害A（此时保险不赔）。E说伤害属于accident（此时保险赔）
...
- insured的cooperate duty涉及多个方面，如compliance with an insurers reasonable request，production of documents and evidence，examination under oath or other types of statements，participation in and independent medical examination，and acceptance of consequences for noncooperation
...
- `property claim中，大部分claim rep要求insured做EUO前，提交sworn proof of loss`
...
- 如果只依赖claim fact，则确定claim是否属于fraudulent会非常困难
...
The claims representative should investigate any discrepancy between the agency information provided by the loss notice form and the producer who submitted the application.
